...
public class MySQLHook implements IDatabaseHook {
    ...
    public void setLink(UUID id, String email) {
        Connection conn = getConnection();
        try (PreparedStatement statement = conn.prepareStatement("INSERT INTO OauthMC (UUID, Email) VALUES (?, ?) ON DUPLICATE KEY UPDATE Email = VALUES(Email)")) {
    ...
    public void createTable() {
        CompletableFuture.runAsync(() -> {
            Connection conn = getConnection();
            try (PreparedStatement statement = conn.prepareStatement("CREATE TABLE IF NOT EXISTS OauthMC (UUID Char(36) PRIMARY KEY, Email VarChar(254) UNIQUE, LinuxDoId VarChar(100), LinuxDoUsername VarChar(100), LastAuthTime BIGINT DEFAULT 0)")) {
                statement.executeUpdate();
            } catch (SQLException ex) {
